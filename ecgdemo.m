%   EKGDEMO    EKG PROCESSING DEMONSTRATION - R-PEAKS DETECTION
%              
%              This file is a part of a package that contains 5 files:
%
%                     1. EKGdemo.m - (this file) main script file;
%                     2. EKGdemowinmax.m - window filter script file;
%                     3. EKGdemodata1.mat - first EKG data sample;
%                     4. EKGdemodata2.mat - second EKG data sample;
%                     5. readme.txt - description.
%
%              The package downloaded from http://www.librow.com
%              and edited by Katie Clary for the purpose of this analysis.
...
function ecgdemo(EKG)
    ...
    %   Original input EKG data
    subplot(3, 2, 1); plot((EKG-min(EKG))/(max(EKG)-min(EKG)));
    title('\bf1. Original EKG'); ylim([-0.2 1.2]);
    %   EKG with removed low-frequency component
    subplot(3, 2, 2); plot((corrected-min(corrected))/(max(corrected)-min(corrected)));
    title('\bf2. FFT Filtered EKG'); ylim([-0.2 1.2]);
    %   Filtered EKG (1-st pass) - filter has default window size
    subplot(3, 2, 3); stem((filtered1-min(filtered1))/(max(filtered1)-min(filtered1)));
    title('\bf3. Filtered EKG - 1^{st} Pass'); ylim([0 1.4]);
    %   Detected peaks in filtered EKG
    subplot(3, 2, 4); stem(peaks1);
    title('\bf4. Detected Peaks'); ylim([0 1.4]);
    %   Filtered EKG (2-d pass) - now filter has optimized window size
    subplot(3, 2, 5); stem((filtered2-min(filtered2))/(max(filtered2)-min(filtered2)));
    title('\bf5. Filtered EKG - 2^d Pass'); ylim([0 1.4]);
    ...
    %   Plotting EKG in green
    plot((EKG-min(EKG))/(max(EKG)-min(EKG)), '-g'); title('\bf Comparative EKG R-Peak Detection Plot');
    %   Show peaks in the same picture
    hold on
    %   Stemming peaks in dashed black
    stem(peaks2'.*((EKG-min(EKG))/(max(EKG)-min(EKG)))', ':k');
    %   Hold off the figure
    hold off
end